...
describe("tool line labels", () => {
  ...
  it("names pid-mcp native tools as MCP calls when the session reports the server", () => {
    const servers = ["klook-engine", "klook", "serena"];
    const l = label(call("klook-engine_lookup_mysql_catalog", { service: "x" }), servers);
    expect(l.running).toBe("Calling MCP");
    expect(l.done).toBe("Called MCP");
    expect(l.detail).toBe("klook-engine · lookup mysql catalog");
  ...
  it("splits on the longest matching server prefix", () => {
    expect(splitMcpToolName("klook-engine_query_log", ["klook", "klook-engine"])).toEqual({
      server: "klook-engine",
      tool: "query_log",
    });
    expect(splitMcpToolName("read", ["klook-engine"])).toBeUndefined();
    expect(humanize("lookup_mysql_catalog")).toBe("lookup mysql catalog");
  });
  it("strips dashed/underscored server prefixes", () => {
    expect(shortMcpName("klook_engine", "klook-engine_query_log")).toBe("query_log");
